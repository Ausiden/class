...
class  logistic_classifier():
    ...
    def loss(self,W,X,y,reg,learning_rate):
        ...
        W = W - learning_rate * (1 / num)*(y_predect.T-y).dot(X)
        return loss,W

    def train(self, X, y, X_val, y_val, Learning_rate=1e-3, learning_rate_decay=0.95, reg=1e-4, num_iters=200,
    ...
    def predict(self, X, W):
        ...
        return y_pred
#多参数调优，寻找最优参数
hidden_size = [2,5,10,15,30,50,100,150] #每批数量
results = {}
best_val_acc = 0
best_net = None
learning_rates = np.array([0.7,0.8,0.9,1.0,1.1])*1e-3 #学习率
regularization_strengths = [0.01,0.1,0.25,0.5,0.75,1.0,1.25]    #正则系数
print('running')
for hs in hidden_size:
    for lr in learning_rates:
        for reg in regularization_strengths:
            logis = logistic_classifier()#下面返回的stats里面存的是train的每一批迭代的正确率
            stats,W = logis.train(x_train,y_train,x_test,y_test,num_iters=1500,batch_size=hs,
...
for hs,lr,reg in sorted(results):
    ...
    if val_acc == best_val_acc:
        print('follow is best parameters')
